...
function SuppliersTable() {
  const { searchQuery, suppliers } = useSelector((state) => state.achat);
  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const [openUpdate, setOpenUpdate] = useState(false);
  const [openD, setOpenD] = useState(false);
  const [selectedSupplier, setSelectedSupplier] = useState(0);
  const [updatedSuppliers, setUpdatedSuppliers] = useState(suppliers);
  ...
  useEffect(() => {
    async function fetchSuppliers() {
      try {
        ...
        setUpdatedSuppliers(response.data);
      } catch (error) {
        console.error("Error fetching suppliers:", error);
      }
    }

    fetchSuppliers(dispatch, suppliers);
  }, [suppliers, dispatch]);
              ...
              {updatedSuppliers
                ...
                .map((supplier) => (
                  <tr key={supplier.id}>
                    ...
                    <td className={`p-4 text-center pr-16 max-w-[250px]`}>
                      <Typography
                        variant="small"
                        color="blue-gray"
                        className="font-normal"
                    ...
                    <td className={`p-4 text-right pr-16 pl-14`}>
                      <Typography
                        variant="small"
                        color="blue-gray"
                        className="font-normal"
