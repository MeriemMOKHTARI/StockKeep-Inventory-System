  ...
  function handlePrint(orderId) {
    ...
    axios({
      ...
      .then((response) => {
        ...
        link.href = url;
        link.setAttribute("download", `Bon_de _décharge_n°${orderId}.pdf`);
                ...
                .filter(
                  (order) =>
                    ...
                    (order.id.toString().startsWith(searchQuery) ||
                      ...
                      order.date.toString().startsWith(searchQuery) ||
                      order.type.toString().startsWith(searchQuery))
                )
                .map((order, index) => (
...
export default DischargeVouchers;
